...
# save the measurement statistics to a CSV file
measurement_stats_static.to_csv('measurement_statistics_w_static_correction.csv', index=False)
measurement_stats_dynamic.to_csv('measurement_statistics_w_dynamic_correction.csv', index=False)
...
dc_injection = compare_dc_shift(fw_df, injection=True)
...
fw_output_directory = '/home/alexis/Downloads/map electrodes/fw_data'

if not os.path.exists(fw_output_directory):
    os.makedirs(fw_output_directory)
unzip_files_in_directory(directory, fw_output_directory)

# %% loop over all unzipped files and compute the summary statistics
import glob
def process_all_unzipped_files(directory,
                               output_directory=None):
    ...
    for csv_file in csv_files:
        ...
        fw_df = pd.read_csv(csv_file)
        measurement_stats, fw_df = compute_summary_statistics(fw_df, 
                                                              window_size=0.33, 
                                                              DC_correction='dynamic')
        # Save the measurement statistics to a CSV file
        output_file = os.path.join(output_directory, f"measurement_stats_{os.path.basename(csv_file)}")
        measurement_stats.to_csv(output_file, index=False)
        print(f"Saved measurement statistics to: {output_file}")

stats_output_directory = '/home/alexis/Downloads/map electrodes/measurement_stats'
if not os.path.exists(stats_output_directory):
    os.makedirs(stats_output_directory)
# process_all_unzipped_files(fw_output_directory, stats_output_directory)
# %% loop over all unzipped files and compute the summary statistics

# Loop through all output statistics and for every quadropole, 
# export the variance of the resistance
resistance_variance_output_file = os.path.join(root_directory, 'resistance_variance.csv')

import glob

# Get all CSV files in the directory
csv_files = glob.glob(os.path.join(stats_output_directory, '*.csv'))

# Initialize a list to store the results
results = []

# create a dataframe that has a row for each electrode.
# loop through the data and append the voltage std for each electrode to the dataframe.
# do so by splitting the quadropole column into a, b, m, n columns
for csv_file in csv_files:
    print(f"Processing file: {csv_file}")
    measurement_stats = pd.read_csv(csv_file)
    
    # Extract the quadropole and resistance std
    for index, row in measurement_stats.iterrows():
        quadropole = row['quadropole']
        resistance_std = row['std resistance']
        
        # Split the quadropole into a, b, m, n
        a, b, m, n = quadropole.split('_')
...
# now extract, for each electrode, the cumulative resistance std
resistance_variance_df = pd.DataFrame(results)

# create another df with a unique row for each electrode
# if the electrode is in the quadropole, then append the resistance std to the electrode
electrode_variance = pd.DataFrame(columns=['electrode', 'resistance_std'])
for electrode in ['a', 'b', 'm', 'n']:
    # get the unique values for the electrode
    unique_values = resistance_variance_df[electrode].unique()
    # get the std for each unique value
    for value in unique_values:
        std = resistance_variance_df[resistance_variance_df[electrode] == value]['resistance_std'].mean()
        electrode_variance = pd.concat([electrode_variance, pd.DataFrame({'electrode': [value], 'resistance_std': [std]})], ignore_index=True)

# if duplicate electrodes exist, then take the mean of the resistance std
electrode_variance = electrode_variance.groupby('electrode').mean().reset_index()

# plot the resistance std for each electrode as a bar plot
plt.figure(figsize=(10, 5))
plt.bar(electrode_variance['electrode'], electrode_variance['resistance_std'], color='b')
plt.xlabel('Electrode')
plt.ylabel('Resistance Std (Ohm)')
plt.title('Resistance Std for each Electrode')
